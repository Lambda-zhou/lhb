...
def draw_kline(df, stock_code):
    ...
    # 使用英文标签避免中文显示问题
    ax.set_title(f'Stock Code: {df["stock_code"].iloc[0]} - Price Chart', 
    ...
    # 显示基本统计信息
    print(f"Stock Code: {df['stock_code'].iloc[0]}")
    print(f"Time Range: {df['trade_time'].min().strftime('%Y-%m-%d %H:%M')} to {df['trade_time'].max().strftime('%Y-%m-%d %H:%M')}")
    print(f"High: {max_price:.2f} at {max_time.strftime('%H:%M')}")
    print(f"Low: {min_price:.2f} at {min_time.strftime('%H:%M')}")
    print(f"Current: {df['price'].iloc[-1]:.2f}")
    print(f"Change: {df['change_pct'].iloc[-1]:+.2f}%")